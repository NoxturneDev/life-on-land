...
def render_all_plantuml_diagrams():
    ...
    print("Rendering 5 Real PlantUML Diagrams via PlantUML Engine with ~1 Deflate Header...")
...
@enduml"""
    ...
    for name, puml_code in diagrams:
        encoded = encode_puml(puml_code)
        # Using ~1 deflate header prefix
        ...
        print(f"Fetching real PlantUML diagram with ~1 header: {name}...")
        try:
            ...
            with urllib.request.urlopen(req) as resp:
                ...
                with open(png_path, "wb") as f:
                    f.write(data)
                print(f"SUCCESS: Saved {png_path} ({len(data)} bytes)")

                # Convert to high quality JPG with white background
